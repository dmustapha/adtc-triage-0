...
const MODIFICATION_REASONS: Record<string, string> = {
  ...
  "tests/integration/server.test.ts": "Kept canonical health identity coverage, changed the real model-backed hero loop to require a narrowed assessment payload with no public classification, plan, or clinical instructions, and bound its fixture to the same explicit IPv4 loopback address used by its requests.",
