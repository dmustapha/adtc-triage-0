...
The review rubric requires a real named qualified human and forbids builder/agent self-review. Product evaluation now fails closed on provisional or missing review identity, and the product manifest binds corpus, holdout design, rubric, and method hashes. Final independent focused verification is 18/18, typecheck and 76-entry import verification pass, calibration IDs are 27/27 unique, calibration/holdout overlap is zero, JSON is 3/3, and no inference or holdout inspection occurred. Artifact hashes: calibration `e06a2fd5356bf9f7fb3465e5cc5c9c059da0ede71fa11710f0a3cd29d31d0296`, holdout manifest `df92449b4b2d455a427e3f9e0c9bb107d1cf68f06c0e1d4488aba8c01b0e7c07`, review rubric `b2edc65a0ad9b500d7c7a61370bb2683f94f70bdbcb34f5ed7d3a2aebd7189e1`, method `c0cc0b6a62bd726701b5c2bd0e147518e3183992f487ebb28fddeaac74e439ec`.

### Structured-danger revision Task 8 — profiler metadata

Commit `e56f891` replaces the stale Python/CSV prompts and generic pairing description with exactly two bounded English healthcare raw/profiler prompts. They require observed-fact/uncertainty separation and explain deterministic structured escalation without diagnosing or prescribing. The exact MedPsy identity remains pinned, `language_scope` remains English-only, `african_alpha_claim` remains false, and all four submitter/team placeholders stay explicit final-packaging blockers.

TDD recorded 14 total / 13 pass / 1 expected failure against the stale prompts. Final independent focused verification passes 14/14 with zero failures/skips/todo; metadata JSON, typecheck, and scoped diff checks pass. Hashes: metadata `9e8e7e30d1bf977d3252f3c1a8ebc4b99c603011f502ef88ea2ed5231a12b605`, canonical test `aef2e91ed8dc788dd90133b51590079a4a504f0699e89e15daf2ca9772ba364f`, raw-profiler test `95cf1dbc30d75dd8f91999e12d273ac8efa293a66ced6c8b4f13daf4e58d30b3`.

### Structured-danger revision Task 9 — split evidence workflow

Commit `bcfb84e` adds a raw/profiler-only GitHub Actions workflow on standard public `ubuntu-24.04`, pinned official llama.cpp, exact MedPsy identity, calibration stop, cleanup-before-upload, and a strict six-file evidence allowlist. It does not run or imply QVAC support on Ubuntu. The separate QVAC product producer refuses GitHub Actions, requires a supplied supported-platform evidence artifact, binds exact model/manifest/calibration hashes, records real boundaries/performance/retries/citations/egress, and was not executed.

Task 9 hardening caught two product-contract bugs before commit: mixed deterministic/QVAC batches originally rejected valid pre-model rows, and the first producer draft loaded QVAC before deterministic cases. Focused RED/GREEN now accepts only named deterministic routes with deterministic telemetry while retaining at least one real model-invoked supported case; QVAC acquisition is lazy inside the QVAC branch; the omitted-object and internal-CONFLICT fixtures remain pre-model. Final focused verification is 33/33; typecheck, 76-entry provenance, YAML parse, every workflow shell block, diff/no-weight/cache scans all pass. No push, dispatch, download, inference, model byte, or publication occurred.

### Structured-danger revision Task 10 — local reconciliation

The first fresh full suite found one real active-contract mismatch: Task 8 updated metadata prompts but `config/profiler-prompt-policy.json` still embedded the obsolete Python prompts. A focused RED reproduced it (11 total / 10 pass / 1 fail). Commit `840af49` reconciles only the active profiler policy and adds a regression; the historical finalist generation-policy bytes remain frozen and unchanged.

Final local verification is green: full suite 277 total / 255 pass / 0 fail / 22 skip / 0 todo; strict TypeScript; 76/76 immutable imports from `74424721bc75f564808eacce42d7f7f42676ae0f`; eight active JSON contracts; workflow YAML and all run-block shell syntax; repository diff; no project, tracked, or QVAC-cache GGUF/partial/part; and historical run `32742482642` aggregate `34a740958016b8fead9edbf16483dc41084b1619d891782756411d9ed962ca57`. The 22 skips are exactly 9 un-ingested plan/grounding cases, 3 un-ingested security/offline cases, 3 un-ingested HTTP cases, 4 un-ingested SSE cases, 2 un-ingested triage cases, and 1 absent citation-map dose-safety case.

Local Build is complete but external claim-bearing gates remain blocked: no supported-platform real QVAC calibration, independent holdout producer/content, named clinical review, physical target-laptop evidence, or signed model decision exists. No new workflow dispatch or publication is authorized. Four real submitter/team identity placeholders also block final packaging. The local deterministic UI can be tested now; the all-absent QVAC path truthfully requires canonical model bytes plus an ingested store and was not run in this revision.

### Task 10 compliance reconciliation

The local Build is complete only under a claim-limited engineering scope. Debug through Interrogate DEEP may inspect, test, and harden the local build through conductor FSM and per-phase gates. They may not claim a validated real QVAC clinical workflow, named clinical review, physical Ubuntu target-laptop proof, a signed model decision, or submission readiness. Phase 2 remains unauthorized and no model decision exists.

No external mutation is authorized: no push, workflow dispatch, paid runner or service, model download, evidence publication, video publication, Devpost submission, or destructive action. The pipeline must stop before Demo Rehearsal. Stress and Verify must disposition the `Online` versus offline wording and the pre-run `This ran on the device` wording without promoting an empty store or absent resident model into inference proof.

Compliance-only verification changed no Task 9 workflow, producer, evaluator, or test bytes. HEAD ancestry preserves published evidence head `a366077`, failed workflow run `32742482642`, and all recorded historical evidence. External completion gates remain explicit: supported-platform real QVAC calibration, independently produced sealed holdouts after calibration, named qualified clinical review, physical target-laptop evidence, real submitter identities, and a truthful signed model decision.
