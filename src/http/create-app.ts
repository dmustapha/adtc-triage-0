...
type Dependencies = {
  supervisedWorkflow: {
    ...
    deterministic?(input: unknown): any | null;
  };
...
function triageRoute(deps: Dependencies) {
  return async (request: Request, response: Response): Promise<void> => {
    ...
    const deterministic = deps.supervisedWorkflow.deterministic?.(parsed.data);
    if (deterministic) {
...
function confirmationRoute(deps: Dependencies) {
  return (request: Request, response: Response): void => {
    ...
    if (parsed.data.decision === "REJECT") { response.json({ reviewState: "REJECTED", replayed: consumed.replayed === true }); return; }
    ...
    response.json({ reviewState: "CONFIRMED", replayed: consumed.replayed === true, classification: consumed.binding.classification, protocol: consumed.binding.protocol, ...projected });
  };
}
